...
def closer_city(lat, lon, city_a, city_b):
    ...
    dist_tar_a = distance(target, city_a)
    dist_tar_b = distance(target, city_b)
    if dist_tar_a < dist_tar_b:
...
def sprout_leaves(t, leaves):
    ...
    for branch in branches(t):
        newbranches += [sprout_leaves(branch, leaves)]
    ...
    if len(branches(t1)) >= len(branches(t2)):
        for i in range(len(branches(t1))):
            if i < len(branches(t2)):
                bs += [add_trees(branches(t1)[i], branches(t2)[i])] 
            else:
                bs += [branches(t1)[i]]
    else:
