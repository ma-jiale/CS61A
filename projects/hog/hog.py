...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while k < num_rolls:
        ...
        if outcome == 1:
            ...
            score += outcome
        k += 1
    return pig_out(score, is_one)
    # END PROBLEM 1
    
def pig_out(score, is_one):
    if not is_one:
        return score
    else:
        return 1
...
def gcd(x, y):
    potential_gcd = 1
    final_gcd = 1
    while potential_gcd <= min(x,y):
        if x % potential_gcd == 0 and y % potential_gcd == 0:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while True:
        ...
        player_score += take_turn(player_strategy(player_score, opponent_score), opponent_score, dice)
        if who == 0:
            score0 = player_score
        else:
            score1 = player_score
        ...
        if not extra_turn(player_score, opponent_score):
            who = other(who)
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            current_get = score0 - last_score
            current_score = score0
        else:
            current_get = score1 - last_score
            current_score = score1
        if current_get > running_high:
            print(current_get, "point(s)! The most yet for Player", who)
            return announce_highest(who, current_score, current_get)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while k <= 10:
        averaged_score = averaged_roll_dice(k, dice)
...
def average_win_rate(strategy, baseline=always_roll(10)):
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    roll_0_score = score + free_bacon(opponent_score)
    if extra_turn(roll_0_score, opponent_score):
        return 0
...
def final_strategy(score, opponent_score):
    """When roll 0 once or twice can win the game, just roll 0,
    else use extra_turn_strategy.
    """
    # BEGIN PROBLEM 12
    if score >= 94:
        return 0
    else: 
        return extra_turn_strategy(score, opponent_score, num_rolls=5)  # Replace this statement
    # END PROBLEM 12
