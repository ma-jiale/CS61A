...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0 #  当前place距离Ant的距离
        def helper(place, distance):
            if place == beehive:
                return None
            elif place.bees != [] and self.min_range <= distance <= self.max_range:
                return rANTdom_else_none(place.bees)
            return helper(place.entrance, distance + 1)
        return helper(self.place, distance)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem Optional 4
        "*** YOUR CODE HERE ***"
        if target and not hasattr(target, "has_been_back"):
            apply_status(make_scare, target, 2)
            target.has_been_back = True
        # END Problem Optional 4
