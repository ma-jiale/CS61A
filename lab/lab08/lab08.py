...
def has_cycle(link):
    ...
    linked_lists = []
    while not link is Link.empty:
        if link in linked_lists:
            return True
        linked_lists.append(link)
        link = link.rest
    return False
...
def has_cycle_constant(link):
    ...
    if not link is Link.empty:
        fast = link
        slow = link
        while not fast.rest is Link.empty:
            slow = slow.rest
            fast = fast.rest.rest
            if slow is fast:
                return True
...
def reverse_other(t):
    ...
    def helper(t, is_odd):
        ...
        labels = []
        for sub_tree in t.branches:
            labels.append(sub_tree.label)
        for sub_tree in t.branches:
            if not is_odd:
                sub_tree.label = labels.pop()
            helper(sub_tree, not is_odd)
