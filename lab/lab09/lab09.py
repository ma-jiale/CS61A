...
def subseqs(s):
    ...
    if not s:
        ...
        return insert_into_all(s[0], sub_subseqs) + sub_subseqs
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1 or n == 2:
        return 1
    return sum([num_trees(sub_times + 1)*num_trees(times - sub_times - 1 + 1) for sub_times in range(times)])
...
class Keyboard:
    ...
    def press(self, info):
        ...
        if info in self.buttons.keys():
            ...
            return f'{self.buttons[info].key}'
        return ''
...
def shuffle(cards):
    ...
    shuffled = []
    for i in range(half):
        shuffled.extend([cards[i], cards[half + i]])
    return shuffled
...
def insert(link, value, index):
    ...
    if link.rest is Link.empty:
        ...
        if index == 0:
            temp, link.first = link.first, value
